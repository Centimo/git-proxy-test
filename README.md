...
|-------------------------------------|-------------------------------------------------------------|
| Не существует                       | Проверяет, есть ли репозиторий на источнике; если есть — создаёт зеркало (Forgejo API `POST /repos/migrate`) и возвращает git-`ERR` → клиент повторяет запрос |
...
**Проверка источника перед созданием зеркала.** Прежде чем создать зеркало, прокси
запрашивает у источника `<clone-url>/info/refs?service=git-upload-pack` — тот самый запрос,
с которого начинается клонирование. Если источник отвечает `404` (или `401`/`403` — анонимно
такой репозиторий всё равно не склонировать), клиент получает `404`, и в Forgejo ничего не
создаётся: иначе каждый запрос выдуманного пути оставлял бы навсегда пустой репозиторий.
Недостижимый источник или неожиданный код ответа проверку не заваливают — миграция идёт как
обычно, это защита от несуществующих путей, а не повод ронять первое клонирование из-за сбоя
сети. Проверка стоит одного запроса и делается только когда зеркала ещё нет.
