...
> Механизм переработан в коммите, следующем за `af2ef85` («proxy: ensure mirror freshness on
> info/refs requests») — там использовалось сравнение refs через `git ls-remote`. Текущая версия
> вместо сравнения полагается на прямой запрос синхронизации к Forgejo.
